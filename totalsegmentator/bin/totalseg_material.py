...
Apply exact anatomical materials/colors to objects in a Blender scene.
This uses the exact logic from material_fix.py with proper mappings.
"""
...
def parse_output_filename():
    """Parse output filename from command line arguments"""
    output_filename = "scene-colored.blend"  # Default name
    argv = sys.argv
    if "--" in argv:
...
def create_material(name, color):
    """Create or replace a material with exact color"""
...
# Create all materials with EXACT colors
materials = {
    ...
    # Also add "Adrenall" for the typo compatibility
    "Adrenall": create_material("Adrenall", (0.799999, 0.254006, 0.03054, 1.0)),
}

# Direct mappings from object names to material names
direct_mappings = {
    "skull": "Bone",
    "costal_cartilages": "Bone",  # Already exists - should work
    "sternum": "Bone",
    "left_clavicle": "Bone",
    "right_clavicle": "Bone", 
    "left_scapula": "Bone",
    ...
    "blood_vessel": "Vein",  # ADDED: Generic blood vessel -> Vein (blue)
}
...
for obj in bpy.data.objects:
    ...
    if not material_name:
        obj_name = obj.name.lower().replace("_", " ").replace("-", " ")
